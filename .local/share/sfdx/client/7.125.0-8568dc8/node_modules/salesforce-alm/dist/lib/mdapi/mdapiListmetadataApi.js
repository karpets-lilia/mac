...
exports.MdListmetadataApi = void 0;
...
class MdListmetadataApi {
    ...
    async createOutputJSONResultFile(retrieveTargetPath, resultJson, mdApi) {
        try {
            await core_1.fs.access(path.dirname(mdApi.retrieveTargetPath));
        }
        catch (err) {
        ...
        await core_1.fs.writeFile(fileName, json);
    ...
    async validate(context) {
        ...
        if (options.resultfile) {
            ...
            try {
                const data = await core_1.fs.stat(retrieveTargetPath);
                if (!data.isFile()) {
