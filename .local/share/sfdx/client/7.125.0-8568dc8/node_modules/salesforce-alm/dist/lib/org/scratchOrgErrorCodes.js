...
exports.checkScratchOrgInfoForErrors = (orgInfo, hubUsername, logger) => {
    ...
    if (orgInfo.Status === 'Error' && orgInfo.ErrorCode) {
        ...
        if (message) {
            throw new core_1.SfdxError(message, 'RemoteOrgSignupFailed', [
                messages.getMessage('signupFailedAction', [orgInfo.ErrorCode]),
            ]);
        }
        throw new core_1.SfdxError(messages.getMessage('signupFailed', [orgInfo.ErrorCode]));
    }
    if (orgInfo.Status === 'Error') {
        ...
        throw new core_1.SfdxError(messages.getMessage('signupFailedUnknown', [orgInfo.Id, hubUsername]), 'signupFailedUnknown');
    }
    throw new core_1.SfdxError(messages.getMessage('signupUnexpected'), 'UnexpectedSignupStatus');
};
