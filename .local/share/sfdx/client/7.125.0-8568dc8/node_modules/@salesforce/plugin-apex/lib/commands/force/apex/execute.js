...
const messages = core_1.Messages.loadMessages('@salesforce/plugin-apex', 'execute');
class Execute extends command_1.SfdxCommand {
    ...
    formatDefault(response) {
        let outputText = '';
        if (response.success) {
            outputText += `${utils_1.colorSuccess(messages.getMessage('executeCompileSuccess'))}\n`;
            outputText += `${utils_1.colorSuccess(messages.getMessage('executeRuntimeSuccess'))}\n`;
            outputText += `\n${response.logs}`;
        }
        else {
            const diagnostic = response.diagnostic[0];
            if (!response.compiled) {
                outputText += utils_1.colorError(`Error: Line: ${diagnostic.lineNumber}, Column: ${diagnostic.columnNumber}\n`);
                outputText += utils_1.colorError(`Error: ${diagnostic.compileProblem}\n`);
            }
            else {
                outputText += `${utils_1.colorSuccess(messages.getMessage('executeCompileSuccess'))}\n`;
                outputText += utils_1.colorError(`Error: ${diagnostic.exceptionMessage}\n`);
                outputText += utils_1.colorError(`Error: ${diagnostic.exceptionStackTrace}\n`);
                outputText += `\n${response.logs}`;
            }
        }
        return outputText;
...
Execute.description = utils_1.buildDescription(messages.getMessage('commandDescription'), messages.getMessage('longDescription'));
